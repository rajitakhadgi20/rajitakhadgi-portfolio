...
galleryRouter.get("/", async (req, res) => {
  const rows = await db.prepare("SELECT * FROM gallery ORDER BY sort_order ASC, id ASC").all();
...
galleryRouter.put("/reorder/all", requireAuth, async (req, res) => {
...
galleryRouter.put("/:id", requireAuth, async (req, res) => {
  const existing = await db.prepare("SELECT * FROM gallery WHERE id = ?").get(req.params.id);
...
galleryRouter.delete("/:id", requireAuth, async (req, res) => {
  const info = await db.prepare("DELETE FROM gallery WHERE id = ?").run(req.params.id);
